...
namespace SD.HnD.DAL.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the Forum Entity. It will perform database oriented actions for
	/// a entity of type 'ForumEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class ForumDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public ForumDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "ForumEntity", new ForumEntityFactory())
		{
		}
		...
		internal ForumDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}


		/// <summary>
		/// Retrieves in the calling ForumCollection object all ForumEntity objects which have data in common
		...
		/// <summary>
		/// Retrieves entities of the type 'ForumEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Retrieves in the calling ForumCollection object all ForumEntity objects
		...
		public bool GetMultiUsingUsersWhoStartedThreads(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IEntity userInstance, int pageNumber, int pageSize)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(SD.HnD.DAL.EntityType.ForumEntity);
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiUsingUsersWhoStartedThreads(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IEntity userInstance, IPrefetchPath prefetchPathToUse)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(SD.HnD.DAL.EntityType.ForumEntity);
			RelationCollection relations = new RelationCollection();
			relations.Add(ForumEntity.Relations.ThreadEntityUsingForumID, "Thread_");
		...
		/// <summary>
		/// Deletes from the persistent storage all 'Forum' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="sectionInstance">SectionEntity instance to use as a filter for the ForumEntity objects to delete</param>
		/// <param name="defaultSupportQueueInstance">SupportQueueEntity instance to use as a filter for the ForumEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity sectionInstance, IEntity defaultSupportQueueInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(SD.HnD.DAL.EntityType.ForumEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(sectionInstance, defaultSupportQueueInstance, fields);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="sectionInstance">SectionEntity instance to use as a filter for the ForumEntity objects to update</param>
		/// <param name="defaultSupportQueueInstance">SupportQueueEntity instance to use as a filter for the ForumEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntity entityWithNewValues, ITransaction containingTransaction, IEntity sectionInstance, IEntity defaultSupportQueueInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(SD.HnD.DAL.EntityType.ForumEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(sectionInstance, defaultSupportQueueInstance, fields);
