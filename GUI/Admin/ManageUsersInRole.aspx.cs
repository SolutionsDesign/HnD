...
{
	/// <summary>
	/// Code behind file for the ManageUsersInRole form.
	/// </summary>
	public partial class ManageUsersInRole : System.Web.UI.Page
	{
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				// bind the Roles repeater to a dataview with all roles.
				DataView dvRoles = SecurityGuiHelper.GetAllRolesWithStatisticsAsDataView();
				
				rpRoles.DataSource = dvRoles;
				rpRoles.DataBind();
			}
		}
		...
		{    
			...
			this.rpRoles.ItemCommand += new System.Web.UI.WebControls.RepeaterCommandEventHandler(this.rpRoles_ItemCommand);
		}
		#endregion
		...
		/// to take.
		/// </param>
		private void rpRoles_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e)
		{
			// Check which button was pressed and redirect to the page in question.
			switch(e.CommandName)
			{
				case "AddUsersToRole":
					// user pressed an add users button. 
					Response.Redirect("AddUsersToRole.aspx?RoleID=" + e.CommandArgument);
					break;
				case "RemoveUsersFromRole":
					// user pressed a remove users button.
					Response.Redirect("RemoveUsersFromRole.aspx?RoleID=" + e.CommandArgument);
					break;
			}
		}
	}
